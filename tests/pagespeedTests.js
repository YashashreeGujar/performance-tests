...
function checkPerformanceMetric(metricName, actualValue, expectedThreshold, url) {
    expect(
        actualValue,
        `${metricName} metric for ${url} is expected to be less than or equal to ${expectedThreshold}. Actual: ${actualValue}, Expected: less than or equal to ${expectedThreshold}`
    ).to.be.lessThanOrEqual(expectedThreshold);
}
    ...
    testURLs.forEach(testURL => {
        describe(`Testing URL: ${testURL}`, function () {
            ...
            it('Should have a good First Contentful Paint (FCP) metric', async () => {
                ...
                checkPerformanceMetric('FCP', fcpValue, FCP_THRESHOLD, testURL);
            });
            ...
            it('Should have a good Largest Contentful Paint (LCP) metric', async () => {
                ...
                checkPerformanceMetric('LCP', lcpValue, LCP_THRESHOLD, testURL);
            });
            ...
            it('Should have a good Cumulative Layout Shift (CLS) metric', async () => {
                ...
                checkPerformanceMetric('CLS', clsValue, CLS_THRESHOLD, testURL);
            });
